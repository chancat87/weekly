...
<small>The cover image was taken from a tree in Beijing. It's been a long time since I've been to Beijing. I happened to come here over the weekend as an invited speaker for AICon. The winter in Beijing is really cold with a bit of a breeze, and the trees are bare, giving a very different feeling from the south.</small>
...
1. Added "Split View Preview" mode, supporting simultaneous editing and previewing in left and right panes, with automatic bidirectional scrolling synchronization.
2. Added "Layout Cycle" button, supporting quick switching between sidebar, note list, and focus mode.
3. Added "Clean Orphaned Attachments" feature, one-click scan and removal of unreferenced images and files.
4. Optimized startup experience, eliminated white screen flickering, and the window fades in more elegantly.
5. Comprehensive performance optimization: Note saving debounce, regex caching, scroll position memory, significantly improving fluency.
6. Fixed multiple known issues such as scroll sync jitter, preview rendering failure, and menu search.
