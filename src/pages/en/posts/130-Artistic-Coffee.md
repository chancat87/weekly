---
date: 2023/05/28
---

<img src="https://gw.alipayobjects.com/zos/k/vx/130.jpg" width="800" />

<small>The cover photo shows a coffee shop in Dongxin Hechuang Park. it was a hot day, and I saw a young man playing guitar outside. I had a great "Dirty" coffee there, though I didn't stay long as the place was packed with young women taking photos.</small>

> **Record the down-to-earth trending technologies seen every week, and publish them here after screening. If you find it good, you can follow this weekly to get update notifications.**

## Technical Related

**The State of Node.js Performance 2023**  
<https://blog.rafaelgss.dev/state-of-nodejs-performance-2023>  
Node.js 20 has brought some incredible performance improvements. worth a look if you're managing backend services.

**The Art of Micro-Frontends**  
<https://medium.com/sysco-labs/the-art-of-micro-frontends-5184065ab74b>  
A solid introduction to the principles and implementation of micro-frontend architecture.

**The Book of Modern C++**  
<https://github.com/lkimuk/the-book-of-modern-cpp/releases>  
An open-source book on "Modern C++" featuring 10 themes across 83 chapters. it’s a high-quality collection of deep-dive articles from the past few years—perfect for C++ developers looking for fresh and advanced content.

## Trending Tools

**"Hacking" the Official ChatGPT for API Use**  
<https://github.com/transitive-bullshit/chatgpt-api>  
I was wondering if there was a way to use the official GPT-4, Plugins, and Web Browsing capabilities via an API. I found a project that uses Puppeteer (headless browser) to handle authentication and message passing. it’s a fascinating way to bypass API limitations for personal projects.  
<a href="https://cdn.tw93.fun/blog/upic/AVY7AT.mp4" target="_blank"><video width="800px" autoplay preload loop muted><source src="https://cdn.tw93.fun/blog/upic/AVY7AT.mp4" type="video/mp4"></video></a>

**Godly: High-End Web Design Inspiration**  
<https://godly.website/>  
If you're stuck on a design, head over to Godly. it curates modern, highly interactive web designs with detailed breakdowns of the effects used. it’s an absolute goldmine for designers and frontend developers.  
<a href="https://cdn.tw93.fun/blog/upic/ChBhhI.mp4" target="_blank"><video width="800px" autoplay preload loop muted><source src="https://cdn.tw93.fun/blog/upic/ChBhhI.mp4" type="video/mp4"></video></a>

**LeiaPix: Transform Photos into 3D Animations**  
<https://convert.leiapix.com/>  
A tool that creates a surprising sense of depth by turning 2D images into 3D animations. you can export the results as MP4 or GIF.  
<img src="https://cdn.tw93.fun/blog/upic/jKRFzc.gif" width="800" />

**Just Focus: A Minimalist Rest Reminder**  
<https://getjustfocus.com/>  
I’ve been looking for a tool to prevent eye strain. Just Focus is a free Pomodoro-style timer that shows a full-screen view when it’s time to rest, and provides daily stats to help you review your productivity.  
<img src="https://cdn.tw93.fun/blog/upic/HuXRgi.png" width="800" />

## Just Looking Around

**C-Eval: Evaluating AI Language Models in Chinese**  
<https://cevalbenchmark.com/static/leaderboard_zh.html>  
How well do global AI models handle the Chinese language? C-Eval provides a comprehensive leaderboard based on 14,000 multiple-choice questions across 52 disciplines. fascinating results.  
<img src="https://gw.alipayobjects.com/zos/k/sh/TinySnap-2023-05-25-22.40.52.png" width="800" />

**Ogden’s Basic English: 850 Words for Everything**  
<http://ogden.basic-english.org/>  
The author of "Basic English" discovered that 90% of the concepts in the Oxford Dictionary can be expressed using just 850 words. the goal is to simplify communication while maintaining perfect clarity. A very interesting approach to linguistics.  
<img src="https://gw.alipayobjects.com/zos/k/0e/SCR-20230523-tzo.png" width="800" />

**The Post-COVID Fate of Community Group Buying Leaders**  
A huxiu article on how community group buying leaders are navigating the changing retail landscape in the post-pandemic era.
